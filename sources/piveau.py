...
def search(search_term: str, results):
    try:
        base_url = utils.config["search_url_piveau"]
        url = base_url + search_term  # + "&filter=dataset"
        ...
        if response.status_code == 200:
            ...
            print(search_result)
            total_hits = search_result['result']['count']
            logger.info(f'PIVEAU - {total_hits} hits/records found')
            if total_hits > 0:
                hits = search_result['result']['results']
                print(hits)

            # for hit in hits:
            #     hit_source = hit.get('_source', None)
            #     publication = Article()
            #     publication.source = 'PIVEAU'
        ...
        results['timedout_sources'].append('PIVEAU')
